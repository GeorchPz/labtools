...
from ..uncertainties import round_with_uncertainty, propagate_uncertainty, get_latex_equations
from ..data import get_excel_data_and_uncertainties
from .statistics import calculate_statistics

def calculate_formula_values(vars_str, equation, data_values, data_uncertainties, dependency='Independent'):
    """
    ...
    result_values, result_uncertainties = propagate_uncertainty(
        vars_str, expression, data_values, data_uncertainties, dependency
    )
    
    # Get LaTeX formulas
    _, formula_latex, uncertainty_latex = get_latex_equations(
        vars_str, equation, dependency, print_latex=False
    )
    
    return result_name, result_values, result_uncertainties, [formula_latex, uncertainty_latex]

def apply_formulas_to_dataframe(dataframe, vars_str, equations, dependency='Independent'):
    """
    ...
    data_values, data_uncertainties = get_excel_data_and_uncertainties(vars_str, dataframe)
    ...
    if isinstance(dependency, str):
        dependency = [dependency] * len(equations)

    # Process each equation
    for equation in equations:
        # Calculate formula values
        result_name, values, uncertainties, latex_formulas = calculate_formula_values(
            vars_str, equation, data_values, data_uncertainties, dependency
        )
        
        # Add to the computed dataframe
        computed_df[f'{result_name}_raw']  = values
        computed_df[f'δ{result_name}_raw'] = uncertainties
        computed_df[f'{result_name} (UNITS)'] = round_with_uncertainty(values, uncertainties)
