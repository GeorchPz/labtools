...
def save_figure(fig, title, prefix='', legend=None, dpi=300, transparent=False, directory=None):
    """Save plot with standardized naming"""
    fig_name = title.replace('$','').replace('\n','').replace('\\','').replace(' ','_')
    fig_path = get_file_path(
        f'{prefix}{fig_name}', extension='.png', directory=directory
        )
